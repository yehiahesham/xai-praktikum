...
class DiffusionModel():
    def __init__(self):
        self.text_2_image_model = StableDiffusionPipeline.from_pretrained("CompVis/stable-diffusion-v1-4", torch_dtype=torch.float16)
        self.text_2_image_model = self.text_2_image_model.to("cuda")
    
    def forward(self, text):
        image = self.text_2_image_model(text).images[0]  
        return image